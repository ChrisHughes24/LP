...
\lstset{basicstyle=\normalsize}
\maketitle
\chris{We should mention somewhere that we only did Phase 2 of the textbook simplex}
\begin{abstract}
...
There are existing formalizations of the simplex algorithm in both Coq \cite{DBLP:journals/corr/AllamigeonK17} and Isabelle \cite{10.1007/978-3-642-32759-9_35}. Additionally the Verified Polyhedron Library \cite{VPL} has some formalization related to the simplex algorithm.

The Coq implementation differs from this implementation in that it was not intended to produce code that could be compiled into a reasonably fast executable algorithm but to use the simplex algorithm as a "tool" to be used to prove other theorems. Consequently they took a very different approach to the proof of the algorithm. They used symbolic perturbation as opposed to Bland's rule for the pivot rule, which leads to a more intuitive proof of termination. The approach taken to their proofs is very different
